...
- **Otimização de LLM**: Crie pipelines de LLM com passes de otimização como cache e retentativas para inferência mais rápida e confiável
...
\* Provedores marcados como **Não** em streaming usam a implementação padrão de `ChatProvider::chat_stream`, que retorna `LLMError::Generic("Streaming not supported for this provider")` em vez de entrar em panic.
...
Recursos de aceleração específicos de hardware, como CUDA, Vulkan e Metal, exigem
as toolchains e plataformas locais correspondentes. Ative esses recursos apenas
para o backend específico que você está compilando.
...
### [Pipelines de LLM](examples/pipeline/)

Demonstra pipelines de LLM com passes de otimização como cache e retentativas para melhorar desempenho e confiabilidade.

### [Guardrails](examples/guardrails/)

Demonstra guardrails configuráveis de entrada e saída com políticas Block, Sanitize e Audit usando uma camada LLMLayer no pipeline.
...
- Testes Rust com `cargo test --features full`
- Testes dos bindings Python com `make python-bindings-test`
