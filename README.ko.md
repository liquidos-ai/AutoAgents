...
AutoAgents는 Rust에서 지능형 시스템을 구축하기 위한 모듈식 멀티 에이전트 프레임워크입니다. 타입 안전한 에이전트 모델, 구조화된 도구 호출, 구성 가능한 메모리, 플러그형 LLM 백엔드를 결합합니다. 아키텍처는 서버와 엣지 환경 모두에서 성능, 안전성, 합성 가능성을 목표로 설계되었으며, 고수준 에이전트 하네스의 기반 역할도 합니다.
...
- **LLM 최적화**: 캐시 및 재시도 같은 최적화 패스를 갖춘 LLM 파이프라인으로 더 빠르고 안정적인 추론 수행
...
| 제공자 | Feature | 채팅 | 스트리밍 | 도구 호출 | 구조화 출력 | 비전 / 멀티모달 |
| --- | --- | --- | --- | --- | --- | --- |
| **OpenAI** | `openai` | 예 | 예 | 예 | 예 | 이미지 URL 및 인라인 이미지; PDF는 타입화된 오류로 거부됩니다. |
| **OpenRouter** | `openrouter` | 예 | 예 | 예 | 예 | OpenAI 호환 이미지 입력; PDF는 타입화된 오류로 거부됩니다. |
| **Anthropic** | `anthropic` | 예 | 예 | 예 | 예 | 이미지, 이미지 URL, PDF는 Anthropic 콘텐츠 블록을 사용합니다. |
| **DeepSeek** | `deepseek` | 예 | 예 | 예 | 예 | OpenAI 호환 이미지 입력; PDF는 타입화된 오류로 거부됩니다. |
| **xAI** | `xai` | 예 | 예 | 아니오 | 모델에 따라 다름 | 텍스트 전용 채팅; 멀티모달 입력은 `LLMError::InvalidRequest`를 반환합니다. |
| **Phind** | `phind` | 예 | 아니오* | 아니오 | 아니오 | 텍스트 전용 채팅; 멀티모달 입력은 `LLMError::InvalidRequest`를 반환합니다. |
| **Groq** | `groq` | 예 | 예 | 예 | 예 | OpenAI 호환 이미지 입력; PDF는 타입화된 오류로 거부됩니다. |
| **Google** | `google` | 예 | 예 | 예 | 예 | 인라인 이미지 및 PDF; 이미지 URL은 타입화된 오류로 거부됩니다. |
| **Azure OpenAI** | `azure_openai` | 예 | 아니오* | 예 | 예 | 이미지 URL; PDF 및 원시 인라인 이미지는 타입화된 오류로 거부됩니다. |
| **MiniMax** | `minimax` | 예 | 예 | 예 | 아니오 | OpenAI 호환 이미지 입력; PDF는 타입화된 오류로 거부됩니다. |

### 로컬 제공자

| 제공자 | Crate / Feature | 채팅 | 스트리밍 | 도구 호출 | 구조화 출력 | 비전 / 멀티모달 | 로컬 추론 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **Ollama** | `ollama` | 예 | 예 | 예 | 예 | 모델에 따라 다름 | 예, Ollama 서버 사용 |
| **Mistral-rs** | `autoagents-mistral-rs` | 예 | 예 | 예 | 예 | 비전 모델 지원 | 예, 임베디드 런타임 |
| **Llama-Cpp** | `autoagents-llamacpp` | 예 | 예 | 예 | 예 | projector 파일로 비전 모델 지원 | 예, 임베디드 런타임 |
...
\* 스트리밍이 **아니오**로 표시된 제공자는 기본 `ChatProvider::chat_stream` 구현을 사용하며, panic 대신 `LLMError::Generic("Streaming not supported for this provider")`를 반환합니다.
