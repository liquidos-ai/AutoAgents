...
| 제공자          | 채팅 | 스트리밍 | 도구 호출 | 멀티모달 참고 |
| --------------- | ---- | -------- | --------- | ------------- |
| **OpenAI**      | ✅   | ✅       | ✅        | 채팅 완성에서 이미지 URL 및 인라인 이미지 지원; Responses API를 통한 PDF는 타입화된 오류로 거부됩니다. |
| **OpenRouter**  | ✅   | ✅       | ✅        | OpenAI 호환 이미지 입력; PDF는 타입화된 오류로 거부됩니다. |
| **Anthropic**   | ✅   | ✅       | ✅        | 이미지, 이미지 URL, PDF는 Anthropic 메시지 콘텐츠 블록을 사용합니다. |
| **DeepSeek**    | ✅   | ✅       | ✅        | OpenAI 호환 이미지 입력; PDF는 타입화된 오류로 거부됩니다. |
| **xAI**         | ✅   | ✅       | 아니오    | 텍스트 전용 채팅; 멀티모달 입력은 `LLMError::InvalidRequest`를 반환합니다. 도구 호출은 `LLMError::NoToolSupport`를 반환합니다. |
| **Phind**       | ✅   | 아니오*  | 아니오    | 텍스트 전용 채팅; 멀티모달 입력은 `LLMError::InvalidRequest`를 반환합니다. 도구 호출은 `LLMError::NoToolSupport`를 반환합니다. |
| **Groq**        | ✅   | ✅       | ✅        | OpenAI 호환 이미지 입력; PDF는 타입화된 오류로 거부됩니다. |
| **Google**      | ✅   | ✅       | ✅        | 인라인 이미지 및 PDF 지원; 이미지 URL은 타입화된 오류로 거부됩니다. |
| **Azure OpenAI** | ✅  | 아니오*  | ✅        | 이미지 URL 지원; PDF 및 원시 인라인 이미지는 타입화된 오류로 거부됩니다. |
| **MiniMax**     | ✅   | ✅       | ✅        | OpenAI 호환 이미지 입력; PDF는 타입화된 오류로 거부됩니다. |
