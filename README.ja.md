...
AutoAgents は、Rust で知的システムを構築するためのモジュール型マルチエージェントフレームワークです。型安全なエージェントモデル、構造化されたツール呼び出し、構成可能なメモリ、差し替え可能な LLM バックエンドを統合しています。アーキテクチャは性能・安全性・合成可能性を重視し、サーバーとエッジの両方を対象に設計されており、高レベルなエージェントハーネスの基盤にもなります。
...
| プロバイダー | Feature | チャット | ストリーミング | ツール呼び出し | 構造化出力 | ビジョン / マルチモーダル |
| --- | --- | --- | --- | --- | --- | --- |
| **OpenAI** | `openai` | 対応 | 対応 | 対応 | 対応 | 画像 URL とインライン画像；PDF は型付きエラーで拒否。 |
| **OpenRouter** | `openrouter` | 対応 | 対応 | 対応 | 対応 | OpenAI 互換の画像入力；PDF は型付きエラーで拒否。 |
| **Anthropic** | `anthropic` | 対応 | 対応 | 対応 | 対応 | 画像、画像 URL、PDF は Anthropic のコンテンツブロックを使用。 |
| **DeepSeek** | `deepseek` | 対応 | 対応 | 対応 | 対応 | OpenAI 互換の画像入力；PDF は型付きエラーで拒否。 |
| **xAI** | `xai` | 対応 | 対応 | 非対応 | モデル依存 | テキストのみのチャット；マルチモーダル入力は `LLMError::InvalidRequest` を返す。 |
| **Phind** | `phind` | 対応 | 非対応* | 非対応 | 非対応 | テキストのみのチャット；マルチモーダル入力は `LLMError::InvalidRequest` を返す。 |
| **Groq** | `groq` | 対応 | 対応 | 対応 | 対応 | OpenAI 互換の画像入力；PDF は型付きエラーで拒否。 |
| **Google** | `google` | 対応 | 対応 | 対応 | 対応 | インライン画像と PDF；画像 URL は型付きエラーで拒否。 |
| **Azure OpenAI** | `azure_openai` | 対応 | 非対応* | 対応 | 対応 | 画像 URL；PDF と生のインライン画像は型付きエラーで拒否。 |
| **MiniMax** | `minimax` | 対応 | 対応 | 対応 | 非対応 | OpenAI 互換の画像入力；PDF は型付きエラーで拒否。 |

### ローカルプロバイダー

| プロバイダー | Crate / Feature | チャット | ストリーミング | ツール呼び出し | 構造化出力 | ビジョン / マルチモーダル | ローカル推論 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **Ollama** | `ollama` | 対応 | 対応 | 対応 | 対応 | モデル依存 | Ollama サーバー経由 |
| **Mistral-rs** | `autoagents-mistral-rs` | 対応 | 対応 | 対応 | 対応 | ビジョンモデル対応 | 組み込みランタイム |
| **Llama-Cpp** | `autoagents-llamacpp` | 対応 | 対応 | 対応 | 対応 | projector ファイルでビジョンモデル対応 | 組み込みランタイム |
...
\* ストリーミングが **非対応** と記載されたプロバイダーは、デフォルトの `ChatProvider::chat_stream` 実装を使用し、panic する代わりに `LLMError::Generic("Streaming not supported for this provider")` を返します。
...
### [LLM パイプライン](examples/pipeline/)

キャッシュやリトライなどの最適化パスを備えた LLM パイプラインを示し、性能と信頼性の向上を実演します。

### [ガードレール](examples/guardrails/)
