...
| プロバイダー       | チャット | ストリーミング | ツール呼び出し | マルチモーダル備考 |
| ------------------ | -------- | -------------- | -------------- | ------------------ |
| **OpenAI**         | ✅       | ✅             | ✅             | チャット補完で画像 URL とインライン画像に対応；Responses API 経由の PDF は型付きエラーで拒否。 |
| **OpenRouter**     | ✅       | ✅             | ✅             | OpenAI 互換の画像入力；PDF は型付きエラーで拒否。 |
| **Anthropic**      | ✅       | ✅             | ✅             | 画像、画像 URL、PDF は Anthropic のメッセージコンテンツブロックを使用。 |
| **DeepSeek**       | ✅       | ✅             | ✅             | OpenAI 互換の画像入力；PDF は型付きエラーで拒否。 |
| **xAI**            | ✅       | ✅             | 非対応         | テキストのみのチャット；マルチモーダル入力は `LLMError::InvalidRequest` を返す。ツール呼び出しは `LLMError::NoToolSupport` を返す。 |
| **Phind**          | ✅       | 非対応*        | 非対応         | テキストのみのチャット；マルチモーダル入力は `LLMError::InvalidRequest` を返す。ツール呼び出しは `LLMError::NoToolSupport` を返す。 |
| **Groq**           | ✅       | ✅             | ✅             | OpenAI 互換の画像入力；PDF は型付きエラーで拒否。 |
| **Google**         | ✅       | ✅             | ✅             | インライン画像と PDF に対応；画像 URL は型付きエラーで拒否。 |
| **Azure OpenAI**   | ✅       | 非対応*        | ✅             | 画像 URL に対応；PDF と生のインライン画像は型付きエラーで拒否。 |
| **MiniMax**        | ✅       | ✅             | ✅             | OpenAI 互換の画像入力；PDF は型付きエラーで拒否。 |
