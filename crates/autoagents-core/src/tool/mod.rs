...
/// Parsed schema hook generated by `#[derive(ToolInput)]` for use with `#[tool]`.
///
/// If the compiler reports that `ToolInputSchema` is not implemented, add
/// `#[derive(ToolInput)]` to the struct referenced by `#[tool(..., input = YourArgs)]`.
#[doc(hidden)]
pub trait ToolInputSchema: ToolInputT {
    fn io_schema_value() -> &'static Value;
}
