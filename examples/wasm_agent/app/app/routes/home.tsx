...
export default function Home() {
    ...
    const initializeWorker = async () => {
        try {
            ...
            const newWorker = new Worker("/streamingWorker.js", {type: "module"});
            ...
            newWorker.onmessage = (event) => {
                ...
                switch (type) {
                    ...
                    case "embedding_status":
                        // Handle embedding status messages (like the Candle example)
                        console.log("Embedding status:", message);
                        if (message.includes("Creating")) {
                            setLoadingMessage("Processing image embeddings...");
                        } else if (message.includes("Complete") || message.includes("Already Set")) {
                            setLoadingMessage("Image ready for analysis");
                        }
                        break;
    ...
    const sendMessage = async () => {
        ...
        // Clear prompt and image, then start streaming in worker
        const currentPrompt = prompt;
        setPrompt("");

        try {
            // Send streaming request to worker with image data if available
            worker.postMessage({
                type: "stream_tokens",
                data: {
                    prompt: currentPrompt,
                    image: imageData ? {data: Array.from(imageData), mimeType} : null
                },
            });
